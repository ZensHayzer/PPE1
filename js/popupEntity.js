...
$(document).ready(function()  {
  $('#boutonUtilisateur').on('click', function()  {
    ...
    $('#popup').append('<form method="POST" action="php/traitementConnexion.php">');
    $('#popup').append('<input type="texte" id="username" name="username" class="popupConnect" placeholder="Identifiant" /></br>');
    $('#popup').append('<input type="password" id="password" name="password" class="popupConnect" placeholder="Mot de passe" /></br>');
...
$(document).ready(function()  {
  $('#submitConnect').on('click', function()  {
    append('<?php header("Location:php/traitementConnexion"); ?>');
  });
});
